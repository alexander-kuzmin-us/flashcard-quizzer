# Flashcard Quizzer: project context for Claude

This repository contains the Flashcard Quizzer CLI. When working here:

- Entry point `main.py`; application code in `quizzer/`; shared helpers in `utils/`.
- Quiz modes use the Strategy pattern (`QuizMode` in `quizzer/quiz_engine.py`) and
  are created only through `QuizModeFactory`. Add new modes with
  `@QuizModeFactory.register`; do not add `if mode == ...` branches anywhere.
- Never let a traceback reach the user: raise a domain exception with a friendly
  message (`FlashcardLoadError`, `FileReadError`, `ExportError`).
- Runtime code uses the standard library only (Python 3.10+).

**Definition of Done for every change:**

```bash
black . && isort . && flake8 . && mypy . && bandit -q -r quizzer utils main.py \
  && python -m pytest --cov=. --cov-report=term
```

All must pass, and coverage must stay above 90%. Fix findings by changing code; any
